...
class GazeMouseController:
    def __init__(
        ...
        sensitivity=2.0,  # Added sensitivity parameter
    ):
        """
        ...
        Args:
            ...
            sensitivity (float): Multiplier for eye movement sensitivity (higher = more sensitive).
        """
        ...
        # Initial gaze range values
        self.min_gaze_x = float("inf")
        self.max_gaze_x = float("-inf")
        self.min_gaze_y = float("inf")
        self.max_gaze_y = float("-inf")

        # Disable pyautogui fail-safe
        pyautogui.FAILSAFE = False

        # Stabilization enhancement: Moving average filter
        self.gaze_history = deque(maxlen=5)  # Reduced from 10 to 5 for faster response
        self.jitter_threshold = (
            2  # Reduced from 5 to 2 pixels - allow more subtle movements
        )
        self.last_stable_gaze = None

        # Exponential smoothing parameters
        self.alpha = 0.5  # Increased from 0.3 to 0.5 for faster response
    ...
    def add_calibration_point(self, gaze_point):
        ...
        if self.calibration_point_index < self.calibration_points:
            actual_point = self.get_calibration_positions()[
                self.calibration_point_index
            ]
    ...
    def _compute_mapping(self):
        ...
        gaze_points_homogeneous = np.hstack(
            (gaze_points, np.ones((len(gaze_points), 1)))
        )

        # Solve for the transformation matrix (least squares)
        self.mapping_matrix, _, _, _ = np.linalg.lstsq(
            gaze_points_homogeneous, screen_points, rcond=None
        )
        return True
    ...
    def map_gaze_to_screen(self, gaze_x, gaze_y):
        ...
        if self.is_calibrated and self.mapping_matrix is not None:
            # Update observed gaze range for adaptive mapping
            self.min_gaze_x = min(self.min_gaze_x, gaze_x)
            self.max_gaze_x = max(self.max_gaze_x, gaze_x)
            self.min_gaze_y = min(self.min_gaze_y, gaze_y)
            self.max_gaze_y = max(self.max_gaze_y, gaze_y)

            gaze_point_homogeneous = np.array([gaze_x, gaze_y, 1])
            screen_x, screen_y = np.dot(gaze_point_homogeneous, self.mapping_matrix)

            # Apply sensitivity factor to emphasize eye movements
            center_x, center_y = self.screen_width // 2, self.screen_height // 2
            dx = (screen_x - center_x) * self.sensitivity
            dy = (screen_y - center_y) * self.sensitivity

            enhanced_x = center_x + dx
            enhanced_y = center_y + dy

            return max(0, min(int(enhanced_x), self.screen_width)), max(
                0, min(int(enhanced_y), self.screen_height)
            )
        else:
            # Improved basic mapping if no calibration
            # Use the observed range of gaze coordinates to map to screen
            if self.min_gaze_x < float("inf"):
                # We have some data on gaze range
                gaze_range_x = max(
                    100, self.max_gaze_x - self.min_gaze_x
                )  # Avoid division by zero
                gaze_range_y = max(100, self.max_gaze_y - self.min_gaze_y)

                # Normalize gaze coordinates within observed range
                normalized_x = (gaze_x - self.min_gaze_x) / gaze_range_x
                normalized_y = (gaze_y - self.min_gaze_y) / gaze_range_y

                # Apply sensitivity to mapping
                screen_x = int(self.screen_width * normalized_x * self.sensitivity)
                screen_y = int(self.screen_height * normalized_y * self.sensitivity)
            else:
                # No data yet, use simple scaling
                scale_x = self.screen_width / 640 * self.sensitivity
                scale_y = self.screen_height / 480 * self.sensitivity
                screen_x = int(gaze_x * scale_x)
                screen_y = int(gaze_y * scale_y)

            # Ensure coordinates are within screen bounds
            return max(0, min(screen_x, self.screen_width)), max(
                0, min(screen_y, self.screen_height)
            )
    ...
    def process_frame(self, frame):
        ...
        if len(self.gaze_history) > 2:  # Reduced from 3 to 2 for faster response
            avg_gaze_x = sum(point[0] for point in self.gaze_history) / len(
                self.gaze_history
            )
            avg_gaze_y = sum(point[1] for point in self.gaze_history) / len(
                self.gaze_history
            )

            # Further smooth with exponential smoothing - now more responsive
            if self.exp_smoothed_x is None:
                self.exp_smoothed_x = avg_gaze_x
                self.exp_smoothed_y = avg_gaze_y
            else:
                self.exp_smoothed_x = (
                    self.alpha * avg_gaze_x + (1 - self.alpha) * self.exp_smoothed_x
                )
                self.exp_smoothed_y = (
                    self.alpha * avg_gaze_y + (1 - self.alpha) * self.exp_smoothed_y
                )
        ...
        # Stabilization enhancement: Reduced jitter threshold for more responsive movement
        ...
        if self.is_enabled:
            ...
            # Apply additional smoothing to cursor movement - reduced for responsiveness
            smoothing_weight = min(0.7, self.smoothing_factor)  # Cap maximum smoothing
            smoothed_x = int(
                self.prev_cursor_pos[0] * (1 - smoothing_weight)
                + screen_x * smoothing_weight
            )
            smoothed_y = int(
                self.prev_cursor_pos[1] * (1 - smoothing_weight)
                + screen_y * smoothing_weight
            )
            ...
            if self.dwell_start_pos is None:
                ...
                if distance > self.dwell_radius:
                    ...
                    cv2.circle(
                        frame, (smooth_gaze_x, smooth_gaze_y), 30, (0, 0, 255), 3
                    )
